...
import { simpleLogin, simpleGetTutorials, simpleCompleteTutorial } from '@/services/simpleAuth';
...
const SimpleTest = () => {
  ...
  const testWrongPassword = async () => {
    ...
    try {
      ...
      const response = await simpleLogin(email, "wrongpassword");
      
      console.log("Login response:", response);
      addTestResult('Wrong Password', false, 'Login should have failed but succeeded');
    } catch (error) {
      console.error("Login error (expected):", error);
      const errorMessage = error instanceof Error ? error.message : 'Login failed';
      addTestResult('Wrong Password', true, `Correctly failed: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const testTutorials = async () => {
    try {
      ...
      const response = await simpleGetTutorials();
  ...
  const testCompleteTutorial = async () => {
    try {
      ...
      const tutorialsResponse = await simpleGetTutorials();
      ...
      if (tutorialsResponse.tutorials && tutorialsResponse.tutorials.length > 0) {
        ...
        const response = await simpleCompleteTutorial(tutorialId);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!localStorage.getItem("token") && (
          <Card className="mb-6">
            <CardHeader>
              ...
              <CardDescription>Test simple login functionality</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={testWrongPassword} disabled={loading} variant="outline">
                    Test Wrong Password
                  </Button>
                </div>
              </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>Test simple authentication flow</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2 flex-wrap">
              ...
              <Button onClick={testTutorials} disabled={!localStorage.getItem("token")}>
                Test Get Tutorials
              </Button>
              <Button onClick={testCompleteTutorial} disabled={!localStorage.getItem("token")}>
